...
static void processPosition(VLCkit * pkits)
{
    ...
    const vector<libvlc_time_t>& tempDuration = pkits->durationArr();
    libvlc_time_t totalSecs = tempDuration[pkits->currentIndex()];
    //QString的版本
...
bool VLCkit::play(const QStringList &names, void *hwnd)
{
    ...
    for(int i = 0; i < names.size(); i++ ){
        ...
        libvlc_time_t durationSecs =
                libvlc_media_get_duration(_pMedia) /1000;    //todo
        //qDebug() << "duration = " <<  << endl;
        _durationArr.push_back(durationSecs);
        qDebug() << "durationSecs = " <<durationSecs  << endl;
        libvlc_media_release(_pMedia);
    }
